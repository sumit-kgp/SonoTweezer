...
NR(1:4)=[6,0,0,0];    %Number of transducers i'th ring 
Rot(1:4)=[0,1,0,0];   %Ring i shited (1=yes; 0=no)
Alpha=43;             %Angle distribution w.r.t cap radius [deg]  (Default = 43)
...
[xarray,yarray,zarray,phi,Nt]=transducerloc(NR,AR,Zi,Rot,M,Trap,deg1,deg2,deg3,deg4,deg5,deg6,deg7,deg8,0,0);
...
for l=1:10
    for n=1:gridsize
Fg(l,n)=9.81*(rho0PS-rho0water)*(4/3)*pi*(((partdiam(l))/2000)^3);
    end
end
...
for z=5:(gridsize-5)
        Fztrap(z)=-1000000*(Uxz(z+1,(1*20+gridsize/2))-Uxz(z-1,(1*20+gridsize/2)))/(2*dx);
end
